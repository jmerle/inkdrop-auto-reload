...
const config = {
  reloadImmediately: {
    ...
    description: 'When disabled reloading is postponed until Inkdrop is given focus. Reloading causes Inkdrop to steal focus from other applications, which can be quite annoying.',
    type: 'boolean',
    default: false
  },
  reloadDisabled: {
...
function activate() {
  ...
  watcher.on('add', path => reload(path)).on('change', path => reload(path)).on('unlink', path => reload(path));
}
